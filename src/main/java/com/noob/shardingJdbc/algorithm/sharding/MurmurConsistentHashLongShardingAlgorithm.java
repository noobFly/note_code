 ...
 *
 */
public class MurmurConsistentHashLongShardingAlgorithm implements PreciseShardingAlgorithm<Long> {
	private final ShadingSelector shadingSelector;
	private final static String UNDERLINE = "_";
	
	public  MurmurConsistentHashLongShardingAlgorithm() {
		shadingSelector = new MurmurShadingSelector(ShardingListBuilder.build(ShardingConst.SHARDING_COUNT));
	}

	@Override
	public String doSharding(Collection<String> tableNames, PreciseShardingValue<Long> shardingValue) {
		String suffix = UNDERLINE + shadingSelector.select(shardingValue.getValue().toString()).getShadingCode();
