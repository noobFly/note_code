 ...
 * 但多了很多没必要的置换） 二叉堆 完全二叉树只是在最后一层要么满子节点，要么都是倒数第二层左边的节点开始满起来。      平衡二叉树的左右子树高度差不超过1，但最底层结点不一定集中在最左边，它本质上是二叉搜索树,需要保证有序： left < root < right。
...
public class HeapSort extends AbstractSort {
	...
	/**
	 * 无论是构建大根堆还是小根堆，循环的结束是指针指向了元数组的0下标位置。因为param[0]才是整个树的root。 2*root + 1 与 2*root
	 * + 2 都没办法在没有新数组的情况下成为整个树的root。
	...
	private void createMaxHeap(int[] param, int lastIndex) {
		for (int parent = (lastIndex - 1) / 2; parent >= 0; parent--) {// 从最后一个父节点开始向前siftUp. 保证最大值会swap到param[0]
			int left = 2 * parent + 1;// 左右子节点中值最大的下标
			if (left < lastIndex) {
				// 若左节点的下标小于最大的坐标，说明有右节点
				if (param[left] < param[left + 1]) {
					left++;
				}
			}

			if (param[parent] < param[left]) { // 比对parent与左右子节点中的最大值
				swap(param, parent, left);
			}
		}
	}
