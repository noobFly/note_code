...
public class NioServer {
    ...
    /**
     * 同一个端口只能被一个ServerSocketChannel绑定监听。 否则报错： Exception in thread "main" java.net.BindException: Address already in use: bind
     * <p>
     * 一个ServerSocketChannel只是绑定一个端口，否则报错：    Exception in thread "main" java.nio.channels.AlreadyBoundException
     */
    public static void main(String[] args) throws Exception {
        serverSocketChannel = ServerSocketChannel.open();
        serverSocketChannel.bind(new InetSocketAddress(8080)); // ServerSocketChannel只能绑定一个端口
        // serverSocketChannel.bind(new InetSocketAddress(8088)); // ServerSocketChannel只能绑定一个端口
